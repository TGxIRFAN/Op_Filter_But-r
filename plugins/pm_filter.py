...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, AUTH_GROUPS, P_TTI_SHOW_OFF, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, PICS, REQ_CHANNEL, NOR_IMG
...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("അല്ലയോ അങ്ങുന്നേ🤒\n മറ്റുള്ളവർ 'Request' ചെയ്ത മൂവി യിൽ കുത്തി നോക്കാതെ സ്വന്തം ആയി 'Request' ചെയ്താലും ❤️", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("You Are Using My Old Message🔧, \nPlease Sent Request Again...", show_alert=True)
        return
...
async def cb_handler(client: Client, query: CallbackQuery):
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                await query.message.edit_text(
                    "I'ᴍ Nᴏᴛ Cᴏɴɴᴇᴄᴛᴇᴅ Tᴏ Aɴʏ Gʀᴏᴜᴘs!\nCʜᴇᴄᴋ /Cᴏɴɴᴇᴄᴛɪᴏɴs Oʀ Cᴏɴɴᴇᴄᴛ ᴛᴏ Aɴʏ Gʀᴏᴜᴘs",
                    quote=True
                )
                return await query.answer('Piracy Is Crime')
    ...
    if query.data.startswith("file"):
        ...
        if str(grp_id) != str(grpid):
            await query.message.edit("Yᴏᴜʀ Aᴄᴛɪᴠᴇ Cᴏɴɴᴇᴄᴛɪᴏɴ Hᴀs Bᴇᴇɴ Cʜᴀɴɢᴇᴅ. Gᴏ Tᴏ /settings.")
            return await query.answer('Piracy Is Crime')
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        ...
        master =  await message.reply_photo(photo=random.choice(NOR_IMG), caption=cap, reply_markup=InlineKeyboardMarkup(btn))
        await message.delete()
